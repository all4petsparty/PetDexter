...
import { useAppStore, type PetCard, type Rarity, type Species } from "@/lib/store";
...
/** Randomized twinkling stars around the floating cutout. */
...
/** Multi-stage capture celebration: catch minigame → FOUND banner → trading card. */
export default function CardReveal() {
  const outcome = useAppStore((s) => s.lastCaptureOutcome);
  const setOutcome = useAppStore((s) => s.setLastCaptureOutcome);
  const setActiveView = useAppStore((s) => s.setActiveView);
  const collection = useAppStore((s) => s.collection);

  const [stage, setStage] = useState<"catch" | "found" | "card">("catch");
  const [caught, setCaught] = useState(false);

  // Treat-drag state (pointer events, mobile-friendly)
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-ink/60 p-6 backdrop-blur-sm">
        <div className="animate-pop-in relative flex w-full max-w-sm flex-col items-center gap-3 overflow-hidden rounded-card bg-white p-8 text-center shadow-2xl">
          ...
          <div className="flex gap-2">
            <span className="rounded-full bg-sunny px-3 py-1 text-sm font-extrabold text-ink">
              XP +{outcome.xp}
            </span>
            <span className="rounded-full bg-bubblegum/30 px-3 py-1 text-sm font-extrabold text-bubblegum">
              🍬 +3
            </span>
          </div>
          <button
            type="button"
            onClick={dismiss}
            className="tappable mt-2 rounded-full bg-grass px-8 py-3 font-extrabold text-white shadow-md"
          >
            Sweet!
          </button>
        </div>
  ...
  const sticker = card.cutoutUrl ?? null;
  const treat = TREAT[card.species];
  ...
  function onTreatPointerUp(e: React.PointerEvent) {
    ...
    if (hit) {
      setCaught(true);
      setTimeout(() => setStage("found"), 650); // let the munch animation play
    }
  }

  // Stage 1 — the catch: floating sticker + drag the treat onto it
  if (stage === "catch") {
    return (
      <div className="fixed inset-0 z-[60] flex flex-col items-center justify-between overflow-hidden bg-ink/85 p-6 backdrop-blur-md">
        <p className="mt-6 animate-pop-in text-center text-xl font-extrabold text-white drop-shadow">
          {caught ? "Gotcha! 🎉" : `A wild ${card.species} appeared!`}
        </p>
        ...
        <div ref={dropZoneRef} className="relative flex h-72 w-72 items-center justify-center">
          ...
          <img
            src={sticker ?? card.imageUrl}
            alt={card.species}
            className={`max-h-64 max-w-64 object-contain ${
              sticker ? "sticker" : "rounded-card"
  ...
  // Stage 2 — FOUND banner with rewards (video-style)
  if (stage === "found") {
    return (
      <button
        type="button"
      ...
      >
        <div className="relative flex h-60 w-60 items-center justify-center">
          ...
          <img
            src={sticker ?? card.imageUrl}
            alt={card.species}
            className={`max-h-56 max-w-56 object-contain ${sticker ? "sticker" : "rounded-card"} animate-bob`}
          />
        </div>

        <div className="animate-pop-in w-full max-w-xs rounded-card border-4 border-grass bg-cream p-5 text-center shadow-2xl">
          <p className="text-sm font-extrabold tracking-widest text-grass-deep">
            ★ {speciesName} FOUND ★
          </p>
  ...
  // Stage 3 — the trading card: sticker on a painterly meadow + battle panel
  return (
      ...
      <div className="animate-pop-in w-full max-w-sm">
        <div className={`rounded-card bg-gradient-to-br p-2 shadow-2xl ${RARITY_GRADIENT[card.rarity]}`}>
          <div className="overflow-hidden rounded-[1.4rem] bg-white">
            {/* Art area: cutout standing in the meadow */}
            <div className="card-meadow relative flex aspect-square items-end justify-center overflow-hidden">
              ...
              <img
                src={sticker ?? card.imageUrl}
                alt={card.customName}
                className={`max-h-[82%] max-w-[86%] object-contain ${sticker ? "sticker" : "rounded-t-card object-cover"}`}
              />
            ...
            {/* Battle panel (video-style scoring) */}
            <div className="flex flex-col gap-2.5 p-4">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {tribes.map((t) => (
                  <span key={t} className="rounded-full bg-grass/20 px-2.5 py-0.5 text-xs font-bold text-grass-deep">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <div className="rounded-2xl bg-cream p-3">
                <span className="rounded-full bg-sunny px-2.5 py-0.5 text-xs font-extrabold text-ink">
                  {ability.keyword}
                </span>
